...
1.**JVM** : It's just the engine that translates Java bytecode (compiled by the JDK) into machine code for execution. Virtual machine that run the Java bytecode.Makes java portable.</br>
 Components:
...
2. **JRE** : JRE is sufficient if you only need to run Java programs but not develop them. It's aimed at end-users who run Java applications. </br>
Components:
- JVM (Java Virtual Machine)
- Core Java libraries and classes
- Supporting files for executing Java applications.
3. **JDK** : The JDK is essential when you need to write, compile, and debug Java code. It provides all the tools required for Java application development.</br>
Components:
- Java compiler (javac)
- Java debugger (jdb)
